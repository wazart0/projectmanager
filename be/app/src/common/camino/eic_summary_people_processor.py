...
def get_eic2025summary_people_report():

    file_path = f"{CONFIG['APP_PATH']}/local/tmp/eic2025summary-people.ods"
    ...
    def get_team_cost_report(spreadsheet):
        ...
        if team_cost_report:
            ...
            if extracted_data:
                ...
                df = df[df['id'] != '']
                log.info(f"Extracted {len(df)} rows of data")
                return df
        else:
            log.error("Table 'Team_total_cost' not found in the document")
            return None
    ...
    def get_time_report(spreadsheet, team_ids, number_of_months):
        ...
        for table in tables:
            ...
            if len(rows) <= row_offset + number_of_months + 1:
                log.error(f"Table {sheet_name} doesn't have enough rows to extract row 8")
                continue
    ...
    return {
        'costs_report': df_costs_long,
        'time_report': df_time_report
    }



def get_other_expenses_report():
    df = pd.read_csv(f"{CONFIG['APP_PATH']}/local/tmp/Team - Faktury.csv")
    df = df[df['Data dok.'].notna()]
    df = df[['Dokument', 'Data dok.', 'Treść', 'Suma', 'Task', 'Cost type']]

    # validate the data
    if 0 < df[['Data dok.', 'Treść', 'Suma', 'Task', 'Cost type']].isnull().sum().sum():
        raise HTTPException(status_code=400, detail="Missing required data in the CSV file")
